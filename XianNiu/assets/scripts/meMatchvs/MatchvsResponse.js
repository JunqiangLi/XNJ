...
MatchvsResponse.prototype.gameServerNotify = function (eventInfo) {
    this.context.node.emit(msg.MATCHV_GANESERVER_NOTIFY, eventInfo);
};
 ...
 * @param roomInfo
 */
MatchvsResponse.prototype.createRoomResponse = function(status, userInfoList, roomInfo){
    this.context.node.emit(msg.MATCHVS_CREATE_ROOM, status, userInfoList, roomInfo);
};
/**
 * 获取房间详情回调
 */
MatchvsResponse.prototype.getRoomDetailResponse = function(MsGetRoomDetailRsp){
    this.context.node.emit(msg.MATCHVS_ROOM_DETAIL, MsGetRoomDetailRsp);
};
